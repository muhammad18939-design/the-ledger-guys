...
const Sidebar = ({ activeTab, setActiveTab, isCollapsed, setIsCollapsed, isMobileOpen, setIsMobileOpen }) => {
  ...
  return (
    <div 
      // Changed to standard ease-in-out and duration-500 for guaranteed smoothness
      className={`fixed left-0 top-0 h-screen text-white flex flex-col transition-all duration-500 ease-in-out z-[100] shadow-2xl border-r border-slate-700/50
    ...
    >
      ...
      <button
        ...
        className="hidden md:flex absolute -right-4 top-10 bg-gradient-to-br from-[#D4AF37] via-[#F3E5AB] to-[#B8860B] text-[#0a1d37] w-8 h-8 rounded-full shadow-[0_0_15px_rgba(212,175,55,0.4)] items-center justify-center border-2 border-white z-[110] hover:scale-110 cursor-pointer transition-all duration-300"
      >
        <ChevronLeft 
          size={18} 
          strokeWidth={3} 
      ...
      {/* Logo Section */}
      <div className={`border-b border-slate-700/50 flex flex-col items-center transition-all duration-500 ease-in-out overflow-hidden ${
        isCollapsed ? 'py-6 px-2 h-24 justify-center' : 'p-6 h-[180px] justify-start'
      }`}>
        <img 
          src="./logo.jpg" 
          alt="The Ledger Guys Logo" 
          className={`rounded-full border-[#D4AF37] shadow-[0_0_15px_rgba(212,175,55,0.2)] transition-all duration-500 ease-in-out object-cover bg-white shrink-0 ${
            isCollapsed ? 'w-10 h-10 border-2' : 'w-20 h-20 border-4'
          }`} 
        />
        
        {/* Animated Text Container */}
        <div className={`flex flex-col items-center transition-all duration-500 ease-in-out overflow-hidden ${
        ...
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* Animated Label */}
              <div className={`transition-all duration-500 ease-in-out overflow-hidden whitespace-nowrap flex items-center ${
      ...
      {/* Footer */}
      <div className={`border-t border-slate-700/50 text-xs text-slate-400 text-center font-mono whitespace-nowrap transition-all duration-500 ease-in-out overflow-hidden ${
